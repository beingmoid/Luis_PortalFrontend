...
export class HistoryDocumentComponent implements OnInit {
  ...
  @Input() searchDocument: string = "";
  visible = false;
  selectedCol: string = "";
  columnsList: any[] = ["Name", "AuditedBy", "AuditDate", "AuditType"];

  @Input() filterDocumentObserver: Subject<boolean>;
  @Input() exportDocumentObserver: Subject<boolean>;
  constructor(private auditService: AuditService, private exportService: ExportService) { }

  ngOnInit(): void {
    //this.auditService.getCaseHistory();
    this.auditService.documentHistoryObserver$.subscribe(res => {
      if(res){
    ...
    this.filterDocumentObserver.subscribe(res => { 
      if(res)
        this.search();
    });
    this.exportDocumentObserver.subscribe(res => {
      if(res)
        this.exportToExcel();
    });
  }
  ...
  getData(params: NzTableQueryParams) {
    ...
    this.params['filterValue'] = this.searchDocument;
    this.auditService.getDocumentHistory(params);
  }
  ...
  exportToExcel(): void {
    ...
    const udt: ExcelJson = {
      data: [
        { A: 'Document Type', B: 'Created/Updated By', C: 'Date & Time', D: 'Description' }
      ],
      skipHeader: true
    };
    ...
    edata.push(udt);
    this.exportService.exportJsonToExcel(edata, 'document_history');
  }
}
